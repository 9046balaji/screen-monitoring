    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Download your digital wellness data as PDF</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Daily Card */}
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 hover:border-blue-500/50 transition-colors rounded-2xl p-6 group">
          <div className="flex items-start justify-between mb-4">
            <div className="bg-blue-500/20 p-3 rounded-xl text-blue-400">
              <FileBarChart2 size={32} />
            </div>
            <span className="text-xs font-semibold px-2 py-1 bg-slate-800 text-slate-300 border border-slate-700 rounded-full">TODAY</span>
          </div>
          <h2 className="text-xl font-bold text-slate-100 mb-1">Daily Summary</h2>
          <p className="text-slate-400 text-sm mb-6 h-10">Complete breakdown of today's app usage and high-risk activity.</p>
          <button 
        ...
        {/* Weekly Card */}
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 hover:border-indigo-500/50 transition-colors rounded-2xl p-6 group">
          <div className="flex items-start justify-between mb-4">
            <div className="bg-indigo-500/20 p-3 rounded-xl text-indigo-400">
              <FileBarChart2 size={32} />
            </div>
            <span className="text-xs font-semibold px-2 py-1 bg-slate-800 text-slate-300 border border-slate-700 rounded-full">LAST 7 DAYS</span>
          </div>
          <h2 className="text-xl font-bold text-slate-100 mb-1">Weekly Deep-Dive</h2>
          <p className="text-slate-400 text-sm mb-6 h-10">7-day trends, wellness score tracking, and automated recommendations.</p>
          <button 
      ...
      {/* Preview Section */}
      <div className="mt-8 bg-slate-800/50 border border-slate-700 rounded-xl p-6 shadow-sm backdrop-blur-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700 flex flex-col justify-center">
            <span className="text-xs text-slate-400 mb-1 font-medium select-none">Top Apps</span>
            {topApps.length > 0 ? (
              <div className="space-y-1">
                {topApps.map((a, i) => (
                  <div key={i} className="text-sm text-slate-300 truncate">• {a.name}</div>
                ))}
              </div>
            ) : (
              <span className="text-xl font-bold text-slate-300">None yet</span>
            )}
          </div>
        </div>
      </div>
