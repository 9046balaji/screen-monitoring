...
export default function DetoxChallenge() {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64 text-slate-600 dark:text-slate-400">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      {/* Progress Header */}
      <div className="bg-surface border border-slate-300 dark:border-slate-700 rounded-3xl p-8 shadow-xl">
          ...
          <motion.div
            ...
            className={`group cursor-pointer p-6 rounded-2xl border transition-all duration-300 flex items-center justify-between gap-4 ${
              task.completed
                ...
                : 'bg-surface border-slate-300 dark:border-slate-700 hover:border-primary/50'
            }`}
          >
            <div className="flex items-center gap-5">
              <div className="flex-shrink-0">
                {task.completed ? (
                  ...
                  <Circle className="w-8 h-8 text-slate-600 dark:text-slate-500 group-hover:text-primary transition-colors" />
                )}
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Calendar className="w-4 h-4 text-slate-600 dark:text-slate-400" />
                  <span className={`text-sm font-semibold uppercase tracking-wider ${task.completed ? 'text-success' : 'text-slate-600 dark:text-slate-400'}`}>
                    Day {task.day}
                  </span>
                </div>
                <h3 className={`text-lg font-medium ${task.completed ? 'text-slate-700 dark:text-slate-300 line-through' : 'text-slate-900 dark:text-white'}`}>
                  {task.task}
                </h3>
              </div>
            </div>
